...
function [ppx,ppy,ppz,err]=splineInterp(X,Y,Z,Tsample,circular)
...
if(nargout>3)%calculate the error
    err=0;
    if(Tsample==1)
        return
    end
    for i=t(1:(end-1))%for each piece
        ...
        z=ppval(ppz,linspace(i,i+1,capprox));
        for j=1:Tsample%for each point
            if(i*floor(length(X)/Tsample)+j>length(X))
                break;
            end
            err= err + min((x-X(i*floor(length(X)/Tsample)+j)).^2+(y-Y(i*floor(length(Y)/Tsample)+j)).^2+(z-Z(i*floor(length(Z)/Tsample)+j)).^2);
        end
    end
    err = err / length(X);
end
