...
function [ppcx,ppcy,ppcz]=computeCenter(ppx,ppy,ppz,Xtr,Ytr,Ztr,Tsample)
...
for i=2:nbreaks-1
    ...
    [~,b]=min((Xj-cX).^2+(Yj-cY).^2+(Zj-cZ).^2);%find closest point
    ...
    Zref=ppval(ppz,linspace(Pprev,Pnext,refine));
    [~,d]=min((Xref-cX).^2+(Yref-cY).^2+(Zref-cZ).^2);%find closest point
%    bla(i)=Pnext;
